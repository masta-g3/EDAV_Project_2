...
p1a = ggplot(data = bind, aes(x = year, y = large))+geom_line(size = 3, alpha = 0.5, color = '#66b3ff')+theme_bw()+ ggtitle('M4 Floods by Years') + ylab('# of floods') 

bind2 = data.frame(annual_num_extreme_flood_yr$Began_yr, annual_num_extreme_flood_yr$`n()`)
colnames(bind2) = c('year','extreme')
p1b = ggplot(data = bind2, aes(x = year, y = extreme))+geom_line(size = 3, alpha = 0.5, color = '#ff6600')+theme_bw()+ ggtitle('M6 Floods by Years') + ylab('# of floods')

grid.arrange(p1a, p1b, ncol = 1)
```

We gather daily gridded geopotential height data(phi) from http://iridl.ldeo.columbia.edu/SOURCES/.NOAA/.NCEP-NCAR/.CDAS-1/.DAILY/.Intrinsic/.PressureLevel/, which has the longitude 0E to 2.5W and latitude 70N to 10N. Then we plot the average phi through years with the dashed smooth line. And it shows that the annual average phi value is increasing. Also, we have number of M4 and M6 floods through years. We can see from plots that the numbers of M4 or M6 is increasing with some variances. 
...
ggplot(phi_year_70_sub, aes(x = year, y = ave_phi_70))+ geom_point(size = 3, alpha = 0.5, color = '#66b3ff')+geom_smooth(se = F,size = 0.75, color = '#3399ff', linetype = 'dashed') +theme_bw()+ ggtitle('Average Phi by Years 36N-70N') + ylab('Average Phi') 

p2a = ggplot(data = binda, aes(x = year, y = large))+geom_line(size = 3, alpha = 0.5, color = '#66b3ff')+theme_bw()+ ggtitle('M4 by Years 36N-70N') + ylab('# of floods') 

bind2a = data.frame(annual_num_extreme_flood_70$Began_yr, annual_num_extreme_flood_70$`n()`)
colnames(bind2a) = c('year','extreme')
p2b = ggplot(data = bind2a, aes(x = year, y = extreme))+geom_line(size = 3, alpha = 0.5, color = '#ff6600')+theme_bw()+ ggtitle('M6 by Years 36N-70N') + ylab('# of floods')
grid.arrange(p2a, p2b,ncol = 1)

```

As the plots aboved, we can observe the same tendency as the total ones, but the vertical axis scales indicate the less number of M4/M6 floods and smaller average phi values through longitude 0E to 2.5W and latitude 70N to 36N.
...
ggplot(phi_year_35_sub, aes(x = year, y = ave_phi_35))+ geom_point(size = 3, alpha = 0.5, color = '#66b3ff')+geom_smooth(se = F,size = 0.75, color = '#3399ff', linetype = 'dashed') +theme_bw()+ ggtitle('Average Phi by Years 10N-35N') + ylab('Average Phi') 

p3a = ggplot(data = bindb, aes(x = year, y = large))+geom_line(size = 3, alpha = 0.5, color = '#66b3ff')+theme_bw()+ ggtitle('M4 by Years 10N-35N') + ylab('# of floods') 

bind2b = data.frame(annual_num_extreme_flood_35$Began_yr, annual_num_extreme_flood_35$`n()`)
colnames(bind2b) = c('year','extreme')
p3b = ggplot(data = bind2b, aes(x = year, y = extreme))+geom_line(size = 3, alpha = 0.5, color = '#ff6600')+theme_bw()+ ggtitle('M6 by Years 10N-35N') + ylab('# of floods')
grid.arrange(p3a, p3b, ncol=1)

```

As the plots aboved, we can observe the same tendency as the last ones, but the vertical axis scales indicate the more number of M4/M6 floods and larger average phi values through longitude 0E to 2.5W and latitude 35N to 10N than longitude 0E to 2.5W and latitude 70N to 36N .
